...
void callback_dispatcher::dispatch( const std::shared_ptr<const generic_callback_t> &callback,
                                    const std::shared_ptr<int> &param ) const
{
    ...
    std::shared_ptr<int> *sh_param = new std::shared_ptr<int>( param );
...
void callback_dispatcher::process_platform_message( UINT nMsg, WPARAM wParam, LPARAM lParam ) const
{
    if( nMsg == m_messageId )
    {
        ...
        std::shared_ptr<int> *param = (std::shared_ptr<int>*) lParam;
        ...
        std::shared_ptr< const generic_callback_t > sh_callback = wk_callback->lock();
        if( sh_callback )
        {
